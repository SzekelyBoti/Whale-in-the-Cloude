...
resource "aws_subnet" "public" {
  ...
  tags = merge(var.common_tags, { Name = "${local.name_prefix}-public-${count.index + 1}" })
}

# Private server subnets
resource "aws_subnet" "private" {
  count             = length(var.private_cidrs)
  vpc_id            = aws_vpc.main.id
  cidr_block        = var.private_cidrs[count.index]
  availability_zone = var.azs[count.index]
  tags = merge(var.common_tags, { Name = "${local.name_prefix}-private-${count.index + 1}" })
}
...
resource "aws_subnet" "db" {
  ...
  availability_zone = var.azs[count.index]
  tags = merge(var.common_tags, { Name = "${local.name_prefix}-db-${count.index + 1}" })
}
